...
import com.tlfmj.bao.sampletwo.activity.DetailActivity;
...
public class OneFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.main_fragment_navigator_one, container, false);
        ButterKnife.bind(this, view);
        list = new ArrayList<>();
        adapter = new AbsCommonAdapter<CaiPiaoBean>(getActivity(), R.layout.fragment_list_item, list) {
            @Override
        ...
        OkHttpUtils.get().url(url).build().execute(new ResultCallback(getActivity()) {
            ...
            public BaseJson parseNetworkResponse(Response response) throws Exception {
                ...
                try {
                    ...
                    if (null != App.list && App.list.size() == 0) {
                        App.list.clear();
                        App.list.addAll(list);
                    }
                } catch (Exception var5) {
                    var5.printStackTrace();
                }
